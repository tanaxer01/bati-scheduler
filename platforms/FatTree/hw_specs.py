...
class PlatformSpecs:
    ...
    def assign(self):
        ...
        # Hosts & Routers
        host_types = iter( sum([ [i["type"] for _ in range(i["number"])] for i in self.platform_specs["nodes"] ], []) )
        print(sum([ [i["type"] for _ in range(i["number"])] for i in self.platform_specs["nodes"] ], []))
        for i in sorted(self.platform.nodes, key=lambda x: x.node_type):
            if i.node_type == NodeType.HOST:
                curr = next(host_types)

                i.props = self.files["nodes"][curr]["properties"]
                i.attrs = self.files["nodes"][curr]["attributes"]
        ...
        xml.SubElement(cluster_zone, "router", attrib=OrderedDict({"id": "router_master"}) )
        ...
        ###
        for i in filter(lambda x: x.level == self.platform.levels, self.platform.nodes):
            xml.SubElement(cluster_zone, "link", attrib=OrderedDict({"id": f"router_master-{i.id}", "bandwidth": "1.25GBps", "latency": "500us" }) )
        ###
        ...
        ###
        for i in filter(lambda x: x.level == self.platform.levels, self.platform.nodes):
            route = xml.SubElement(cluster_zone, "route", attrib=OrderedDict({"src": "router_master", "dst": i.id}))
            xml.SubElement(route, "link_ctn", attrib=OrderedDict({"id": f"router_master-{i.id}"}))
        ###
