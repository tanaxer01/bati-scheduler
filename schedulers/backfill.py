from batsim_py.jobs import Job, JobProfile
...
class FreeSpaceContainer(object):
    ...
    def _assignJobBeginning(self, l: FreeSpace, job: Job):
        alloc = (l.first_res, l.first_res + job.res - 1)
        l.first_res = l.first_res + job.res
    ...
    def _assignJobEnding(self, l, job):
        alloc = (l.last_res - job.res + 1, l.last_res)
        l.last_res = l.last_res - job.res
        l.res = l.last_res - l.first_res + 1
        assert l.res >= 0
        if l.res == 0:
            self.remove(l)
    ...
    def assignJob(self, l: FreeSpace, job: Job, current_time: float):
        assert job.res <= l.res
    ...
    def unassignJob(self, job: Job):
        ...
        if mergel1 and mergel2:
            ...
            # increase l2 size
            l2.first_res = l2.first_res - job.res
...
def findAllocFuture(listFreeSpace, runningJobs, job):
    listFreeSpaceTemp = listFreeSpace.copy()
    for j in runningJobs:
        new_free_space_created_by_this_unallocation = listFreeSpaceTemp.unassignJob(j)
        if job.res <= new_free_space_created_by_this_unallocation.res:
...
def allocJobFCFS(listFreeSpace: FreeSpaceContainer, job: Job, current_time: float):
    for l in listFreeSpace.generator():
        if job.res <= l.res:
            alloc = listFreeSpace.assignJob(l, job, current_time)
            return alloc
    return None

def allocFutureJob(listFreeSpace: FreeSpaceContainer, first_job_res, first_job_starttime, current_time):
    """
    Update self.listFreeSpace to insert (if needed) 2 virtual free space.
    These freespaces need to be removes afterwards Example: 3 machine
    (A,B,C), 1 job running (1), the firstjob is 2
...
if __name__ == "__main__":
    a = Job("test_job", "test_workload", 1, JobProfile("test_profile"),  5, 10)
    b = Job("test_job", "test_workload", 1, JobProfile("test_profile"), 10, 10)
    waiting_jobs = [a, b]

    A = FreeSpaceContainer(3)
    A.printme()

    alloc = allocJobFCFS(A, a, 0)

    findAllocFuture(A, [a], Job("test", "test_workload", 1, JobProfile("testProfile"), 5, 10))
    #allocFutureJob(A, 0, 5, 0)
